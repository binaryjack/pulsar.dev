 ...
 */
function renderError(container: HTMLElement, errorInfo: IErrorInfo, props: ICatcherProps): void {
  // Clear container
  container.innerHTML = '';
  ...
  if (props.fallback) {
    ...
    // Handle different types of JSX content
    if (fallbackContent instanceof HTMLElement || fallbackContent instanceof DocumentFragment) {
      container.appendChild(fallbackContent);
    } else if (typeof fallbackContent === 'string' || typeof fallbackContent === 'number') {
      container.textContent = String(fallbackContent);
    }
    return;
  }
