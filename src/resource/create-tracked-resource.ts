 ...
 * ```
 */
export function createTrackedResource<T>(
    fetcher: ResourceFetcher<T>,
    options: IResourceOptions = {}
): IResource<T> {
    // Create base resource with lazy loading
    const resource = createResource(fetcher, { 
        ...options, 
        lazy: true 
    });
    
    // Setup effect to track dependencies and auto-refetch
    createEffect(() => {
        // Execute fetcher which tracks any signals/memos accessed
        resource.load();
    });
